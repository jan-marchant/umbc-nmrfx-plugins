    ...
    void updateButtons(String selGroupEntry) {
        if (selGroupEntry.length() > 0) {
            ...
            for (String selGroupStr : selGroupStrs) {
                RNALabels2.SelGroup selGroup = RNALabels2.parseSelGroup(selGroupStr);
                    ...
                    if (RNALabels2.checkResType(baseChars[iBase], selGroup.resTypes)) {
                        for (int iType = 0; iType < 3; iType++) {
                            for (CheckBox checkBox : checkBoxes[iBase][iType]) {
                                ...
                                boolean state = RNALabels2.checkAtom(checkBox.getText(), checkBox.getText().substring(0, 1), selGroup.gAtomNames, ribose, exchangeable);
                                if (state) {
